...
4. **knowledge/ entries require YAML frontmatter.** Every file under `knowledge/`
   needs `source`, `processed`, and `method` fields or you'll get warnings.

5. **`updates/` and `decisions.md` are append-only.** Use `append_to_file`, never
   `write_file`, for those paths.

6. **Register M365 sources before using M365 refs.** If you're linking content to a
...
7. **Filenames must be kebab-case.** `email-threads.md` ✓  `Email Threads.md` ✗

Once you've read both reference files, proceed with the user's request using the
appropriate tools.
